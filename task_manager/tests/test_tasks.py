...
def test_task_creation(client, task_data, subtask_form_data):
    response = client.post(reverse("create_task"),
                           data=dict(**task_data, **subtask_form_data))
    assert response.status_code == 302
    assert TaskModel.objects.count() == 1, ("Убедитесь,"
                                            " что задача создаётся корректно")
    assert SubTask.objects.count() == 1, ("(Убедитесь,"
                                          " что подзадача создаётся корректно")
...
def test_task_update_valid_data(client, created_task):
    response = client.post(reverse("update_task", kwargs={"task_id": 1}),
                           data=dict(
                               name="Test task",
...
@pytest.mark.django_db
def test_task_update_completes_when_in_progress(
        client,
        task_in_progress
):
    response = client.post(reverse("update_task", kwargs={"task_id": 1}),
